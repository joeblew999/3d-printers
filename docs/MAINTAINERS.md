...
5. Regenerate docs with correct URLs:
   ```sh
   task docs:generate
   ```
6. Push and tag `v0.1.0` to trigger first release
...
cmd/
  ...
  docgen/          # Generates docs/index.md from Taskfile vars
internal/
...
- Run `task docs:generate` after changing Taskfile vars or template
